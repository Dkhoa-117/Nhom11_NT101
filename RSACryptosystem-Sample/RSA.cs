...
namespace RSA
{
    public partial class RSACryptosystem : Form
    {
        ...
        private string EncryptedPath = ""; //chua duong dan file MaHoa 
        private string DecryptedPath = ""; //chua duong dan file GiaiMa
        // sau khi giai ma
        ...
        private void Encrypt()
        {
                ...
                if (this.PublicKeyPath.Length != 0 || this.KeyPath.Length != 0)
            {
                if (tbInput.Text.Length != 0)
                {
                    ...
                    bool isValidated = RSA_Algorithm(inputFileName, outputFileName, RSA.ExportParameters(false), true);
        ...
        private void Decrypt()
        {
            ...
            try
            {
                if (tbInput.Text.Length != 0)
                {
                    ...
                    if (Path.GetExtension(inputFileName) != ".nhom11")
                    {
                    ...
                    bool isValidated = RSA_Algorithm(inputFileName, outputFileName, RSA.ExportParameters(true), false);
        ...
        private bool RSA_Algorithm(string inputFile, string outputFile, RSAParameters RSAKeyInfo, bool isEncrypt)
        {
            try
            {
                ...
                if (totlen <= maxBytesCanEncrypted) // 0  64 128 192 224 //4 1024 1 file 1024 x 4 4096
                {
                    
                    FileStream fsCipherText = new FileStream(outputFile, FileMode.Create, FileAccess.Write); //Ghi file output

                    bin = new byte[totlen];
                    len = fsInput.Read(bin, 0, (int)totlen); // 
                    ...
                    //
                    fsCipherText.Write(encryptedData, 0, encryptedData.Length);
                ...
                {
                    MessageBox.Show("File Size is longer than Key Length !");
        ...
        private void btShowResult_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ioex)
            {
                ...
                try
                {
                    System.Diagnostics.Process.Start("Notepad.exe", DecryptedPath);
                }
                catch (Exception ioex)
                {
                    MessageBox.Show("Failed: " + ioex.Message);
